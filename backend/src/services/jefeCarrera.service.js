...
export async function createJefeCarreraService(data) {
    ...
    //limitar a maximo 3 jefes de carrera
...
export async function getSolicitudesPorCarreraService(jefeId, filtros = {}) {
    try {
        ...
        //filtrar por carrera SOLO si el jefe NO es admin
        ...
        //si jefe.carrera === null, es ADMIN = ve TODAS las solicitudes
        ...
        //obtener informacion de alumno con carrera para cada solicitud
...
export async function cambiarEstadoSolicitudService(solicitudId, nuevoEstado, jefeId) {
    try {
        ...
        //validar que el estado sea valido (solo ACEPTADO o RECHAZADO)
        if (!["ACEPTADO", "RECHAZADO"].includes(nuevoEstado)) {
            return { error: "Estado inválido. Solo se permite: ACEPTADO o RECHAZADO" };
        }
        
        //obtener información del jefe autenticado con su carrera
        const jefe = await usuarioRepository.findOne({
            where: { id: jefeId },
            relations: ["carrera"]
        });
        ...
        //validar que la solicitud = estado PENDIENTE
        ...
        //verificar que el jefe pueda modificar esta solicitud
        //si el jefe (NO es admin), solo puede modificar solicitudes de SU carrera
        if (jefe.carrera !== null) {
        ...
        //jefe.carrera === null, es ADMIN = puede modificar cualquier solicitud
        
        //si el nuevo estado es ACEPTADO, validar que haya cupos disponibles
        if (nuevoEstado === "ACEPTADO") {
            ...
            //contar cuantas solicitudes ACEPTADAS hay actualmente para esta carrera en este electivo
        ...
        //retornar la solicitud actualizada, con nueva info
        return { 
            data: {
...
export async function moverListaEsperaAPendienteService(solicitudId, jefeId) {
    try {
        ...
        //obtener informacion del jefe autenticado con su carrera
        const jefe = await usuarioRepository.findOne({
            where: { id: jefeId },
            relations: ["carrera"]
        });
        ...
        //validar que el estado de la solicitud = LISTA_ESPERA
        ...
        //obtener información del alumno con su carrera
        const alumnoInfo = await alumnoRepository
            .createQueryBuilder("alumno")
            .leftJoinAndSelect("alumno.carrera", "carrera")
            .where("alumno.usuario_id = :usuarioId", { usuarioId: solicitud.alumno.id })
        ...
        //validar que haya cupos disponibles antes de mover a PENDIENTE
        ...
        //mover de LISTA_ESPERA a PENDIENTE
